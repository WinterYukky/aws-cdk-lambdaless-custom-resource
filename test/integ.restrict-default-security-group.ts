...
const workflow = new CustomResourceFlow(stack, 'CustomResourceFlow', {
  ...
  onUpdate: Choice.jsonata(stack, 'New Security Group?')
    .when(
      Condition.jsonata(
        '{% $OldResourceProperties.DefaultSecurityGroupId != $ResourceProperties.DefaultSecurityGroupId %}',
      ),
      new AuthorizeRules(stack, 'AuthorizeRules on Update', {
        vpcDefaultSecurityGroup: vpc.vpcDefaultSecurityGroup,
        resourceProperties: 'OldResourceProperties',
      })
        .prefixStates()
        .next(
          new RevokeRules(stack, 'RevokeRules on Update', {
            vpcDefaultSecurityGroup: vpc.vpcDefaultSecurityGroup,
            resourceProperties: 'ResourceProperties',
          }).prefixStates(),
        ),
    )
    .otherwise(Pass.jsonata(stack, 'No changes')),
  onDelete: new AuthorizeRules(stack, 'AuthorizeRules on Delete', {
    vpcDefaultSecurityGroup: vpc.vpcDefaultSecurityGroup,
    resourceProperties: 'ResourceProperties',
  }).prefixStates(),
});
